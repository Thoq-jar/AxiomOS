...
function checkDependencies(): array {
    $dependencies = [
        ...
        'make' => ['name' => 'Make', 'required' => true],
        'gcc' => ['name' => 'GCC Compiler', 'required' => true],
        'curl' => ['name' => 'cURL', 'required' => true],
        'systemctl' => ['name' => 'Systemd', 'required' => true],
        'docker' => ['name' => 'Docker', 'required' => false]
    ];
...
ExecStart=$projectPath/axiom-os
Restart=always
RestartSec=10
...
function install($selectedComponents, $storagePath, $startWithServer): bool {
    $repoUrl = 'https://github.com/axiom-os/axiom-os.git';
    ...
    if(!empty($missingRequired)) {
        ...
        foreach($missingRequired as $cmd => $info) {
            echo "  ✗ {$info['name']}\n";
        }
...
function waitForInput($message): void {
    echo "$message";
    if(isTTY()) {
        readKey();
    } else {
